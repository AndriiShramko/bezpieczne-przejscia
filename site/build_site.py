...
.vbtn:hover{transform:translateY(-1px);filter:brightness(1.25)}
.vbtn:active{transform:scale(.96)}
.vbtn.mine{outline:2px solid var(--acc)}
.vbtn.voting{animation:vpulse .7s ease-in-out infinite;pointer-events:none}
.vbtn.flash{animation:vflash .45s ease-out}
@keyframes vpulse{0%,100%{opacity:1}50%{opacity:.45}}
@keyframes vflash{0%{transform:scale(1);filter:brightness(2.1)}100%{transform:scale(1);filter:brightness(1)}}
...
.chart-card{background:var(--panel);border-radius:14px;padding:1rem 1.1rem 1.2rem;display:flex;flex-direction:column}
.chart-card h3{font-size:.95rem;margin:.2rem 0 .6rem}
.chart{min-height:158px}.chart .legend{display:flex;gap:.6rem .9rem;flex-wrap:wrap;font-size:.72rem;color:var(--mut);margin-top:.35rem;line-height:1.3}
.chart svg rect,.chart svg circle{transition:opacity .4s ease,transform .4s ease;transform-box:fill-box;transform-origin:bottom}
.chart svg rect{cursor:default}
.chart svg .evbar rect,.chart svg rect[data-hour]{cursor:pointer}
.chart svg rect:hover{filter:brightness(1.25)}
@keyframes growbar{from{transform:scaleY(0);opacity:.2}to{transform:scaleY(1);opacity:1}}
.chart svg.anim rect{animation:growbar .5s ease both}
.chart svg.anim polyline{stroke-dasharray:1400;stroke-dashoffset:1400;animation:drawline 1.1s ease forwards}
@keyframes drawline{to{stroke-dashoffset:0}}
/* share CTA — big, on-brand, works on both languages */
.share-cta{margin:2rem auto;max-width:720px;padding:1.4rem 1.2rem;border-radius:16px;
  background:linear-gradient(135deg,rgba(46,230,166,.12),rgba(55,182,255,.10));
  border:1px solid rgba(46,230,166,.35);text-align:center}
.share-cta h3{margin:0 0 .3rem;font-size:1.25rem}
.share-cta p{margin:.2rem 0 1rem;color:var(--mut)}
.share-cta .btns{display:flex;gap:.7rem;flex-wrap:wrap;justify-content:center}
.share-cta a,.share-cta button{display:inline-flex;align-items:center;gap:.4rem;font:inherit;
  font-weight:700;font-size:1rem;padding:.7rem 1.3rem;border-radius:11px;border:0;cursor:pointer;
  color:#fff;text-decoration:none;transition:transform .12s ease,filter .12s ease}
...
.pl-timer{display:inline-flex;align-items:center;gap:.5rem;margin:.5rem 0;padding:.5rem .9rem;
  ...
  font-size:1rem;font-weight:700;color:#8fd3ff}
.pl-timer b{color:#fff;font-variant-numeric:tabular-nums}
